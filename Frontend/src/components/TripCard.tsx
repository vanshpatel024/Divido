  ...
  if (balance.kind === "owed") {
    return (
      <span
        ...
        style={{
          background: "var(--status-getback-bg)",
          color: "var(--status-getback-text)",
        }}
      >
...
export default function TripCard({ trip, index: _index, onDelete }: TripCardProps) {
  ...
  const leftBorderColor: string = isSettled
    ...
    ? "var(--status-getback-text)"
    : "var(--accent)";

  return (
    <article
      onClick={handleCardClick}
      className="group flex flex-col rounded-2xl border border-border bg-card p-5 shadow-sm border-l-4 relative cursor-pointer select-none transition-all duration-300 hover:border-primary/30 hover:shadow-lg"
