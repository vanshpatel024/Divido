...
export default function MainLayout() {
  ...
  useEffect(() => {
    ...
    activeSpans.forEach((span) => {
      ...
      while (!isValid && retries < 50) {
        startX =
          typeof span.start.x === "number"
            ? span.start.x
            : span.start.x[0] +
              Math.random() * (span.start.x[1] - span.start.x[0]);
        startY =
          typeof span.start.y === "number"
            ? span.start.y
            : span.start.y[0] +
              Math.random() * (span.start.y[1] - span.start.y[0]);
        endX =
          typeof span.end.x === "number"
            ? span.end.x
            : span.end.x[0] + Math.random() * (span.end.x[1] - span.end.x[0]);
        endY =
          typeof span.end.y === "number"
            ? span.end.y
            : span.end.y[0] + Math.random() * (span.end.y[1] - span.end.y[0]);

        // Check if endpoints are too close to any previously generated line's endpoints
        let tooClose = false;
        for (const existing of generated) {
          const distStart = getDistance(
            startX,
            startY,
            existing.start.x,
            existing.start.y,
          );
          const distEnd = getDistance(
            endX,
            endY,
            existing.end.x,
            existing.end.y,
          );
          const distCross1 = getDistance(
            startX,
            startY,
            existing.end.x,
            existing.end.y,
          );
          const distCross2 = getDistance(
            endX,
            endY,
            existing.start.x,
            existing.start.y,
          ...
          if (
            distStart < minDistance ||
            distEnd < minDistance ||
            distCross1 < minDistance ||
            distCross2 < minDistance
          ) {
            tooClose = true;
            break;
          }
  ...
  return (
    <div className="min-h-screen text-foreground font-sans pb-6 relative transition-colors duration-300">
