...
const CATEGORIES: { value: Category; label: string; icon: React.ReactNode }[] = [
  ...
  { value: "shopping", label: "Shopping", icon: <ShoppingBag size={14} /> } as any // Note: Shopping is in TripDetail but not explicitly in Category type, we can cast or handle it
];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const tripData = {
      ...
      participants: participants.map((p, idx) => ({
        name: p,
        color: ["#AAD9BB", "#C9B7E0", "#F7DCB9", "#FBC4AB", "#B7D4E0"][idx % 5]
      })),
    ...
    console.log("Create trip data:", tripData);
    if (onCreate) {
      onCreate(tripData);
    }
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border border-[#EFECE6] bg-white py-2.5 px-3.5 text-sm outline-none transition-all duration-200 hover:border-[#AAD9BB] focus:border-[#AAD9BB] focus:shadow-[0_0_0_3px_rgba(170,217,187,0.25)] text-foreground cursor-pointer"
              />
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-[#8B8A9B] mb-1.5 select-none">
