...
export default function EditStopModal({ isOpen, onClose, onUpdate, participants, stop, isSubmitting = false }: EditStopModalProps) {
  ...
  useEffect(() => {
    if (isOpen && stop) {
      setName(stop.name);
      setAmountInput(stop.total.toString());
      setDate(stop.date.split('T')[0]);
      
      // Parse existing transactions to populate form
      const uniquePayers = [...new Set(stop.transactions.map(t => t.paidBy))];
