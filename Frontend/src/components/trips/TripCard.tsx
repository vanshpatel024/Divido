...
function StatusBadge({ balance }: { balance: Balance }) {
  if (balance.kind === "settled" || (balance as any).amount === 0) {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[11px] font-medium tracking-wide bg-[var(--color-status-settled-bg)] text-[var(--color-status-settled-text)]">
        Settled
      </span>
    );
  }
  if (balance.kind === "owed") {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[11px] font-semibold tracking-wide bg-[var(--color-status-getback-bg)] text-[var(--color-status-getback-text)]">
  ...
  //"owe"
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[11px] font-semibold tracking-wide bg-[var(--color-status-owe-bg)] text-[var(--color-accent)]">
      − {formatInr((balance as any).amount)}
    </span>
  );
}
...
export default function TripCard({
  ...
  const isSettled =
    trip.balance.kind === "settled" || (trip.balance as any).amount === 0;
  ...
  return (
    <motion.article
      ...
      className="group flex flex-col rounded-2xl border border-border bg-card p-5 shadow-sm border-l-4 border-l-[var(--left-border-color)] relative cursor-pointer transition-colors duration-300 hover:border-[var(--hover-border-color)]"
      style={{
